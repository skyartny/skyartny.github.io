...
## 5. 应用定制 (App Customization)

### 更改应用标题或 Logo
您可以自定义应用程序的主标题部分，以显示您组织的名称或徽标。
1. 在 **ROSTER** 中，滚动到底部的 **Data Management** (数据管理) 部分。
2. 点击 **Customize App Title** (自定义应用标题)。
3. 选择 **Text** (文字) 或 **Image** (图片)。
   * **文字**: 输入自定义名称 (例如 "My Studio")。文字将显示出彩色的渐变效果。
   * **图片**: 直接从您的设备上传徽标图像文件。
     * **最佳尺寸**: 280 × 64 像素（宽扁型格式）。
     * **格式建议**: 推荐使用透明背景的 PNG 或 SVG 文件。
     * **最大限制**: 1 MB。
4. 点击 **Save Title** (保存标题)。

### 切换语言
管理员界面支持多种语言。
1. 在 **ROSTER** 中，找到 **Data Management** (数据管理) 部分。
2. 在此部分标题的右上角，找到 **Language** (语言) 下拉菜单。
3. 选择您的首选语言 (例如 English 或 中文)。界面将立即更新，且您的偏好将被保存。
